...
const IconBack = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#004A8F" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M19 12H5M12 19l-7-7 7-7" />
  </svg>
);

const IconClose = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8899AA" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
...
/* Step 1 option icons */
const IconRenovate = () => (
  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#004A8F" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
/* Step 4 field icons */
const IconLocation = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#004A8F" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
const IconNoSpam = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#004A8F" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="12" cy="12" r="10" />
...
const IconCheckLarge = () => (
  <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#008A26" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="12" cy="12" r="10" />
    <path d="M9 12l2 2 4-4" />
  </svg>
);
...
const IconWarningLarge = () => (
  <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#D4782F" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const STEP1_OPTIONS = [
  ...
  { id: 'independent', label: 'unabhängiger von Öl & Gas werden.', Icon: IconShieldCheck },
  { id: 'replace', label: 'eine alte oder defekte Heizung ersetzen.', Icon: IconWrench },
];
        ...
        {STEP1_OPTIONS.map(({ id, label, Icon }) => {
          ...
          return (
            ...
            >
              ...
              <span className="pico-option-icon"><Icon /></span>
...
function Step2({ onSelect }) {
  return (
    <div className="pico-step">
      <h2 className="pico-step-title">Sind Sie Eigentümer der Immobilie?</h2>

      <div className="pico-options pico-options--spaced">
        <button type="button" className="pico-option-card" onClick={() => onSelect(true)}>
          <span className="pico-option-icon"><IconCircleCheck /></span>
          <span className="pico-option-label">Ja</span>
        </button>
        <button type="button" className="pico-option-card" onClick={() => onSelect(false)}>
          <span className="pico-option-icon"><IconCircleX /></span>
...
function Step3({ onSelect }) {
  return (
    <div className="pico-step">
      ...
      <div className="pico-options pico-options--spaced">
        <button type="button" className="pico-option-card" onClick={() => onSelect('online')}>
          <span className="pico-option-icon"><IconMonitor /></span>
          <span className="pico-option-label">Online-Check starten & Preisschätzung erhalten</span>
        </button>
        <button type="button" className="pico-option-card" onClick={() => onSelect('callback')}>
          <span className="pico-option-icon"><IconPhone /></span>
          <span className="pico-option-label">Rückruf anfordern für ein persönliches Gespräch</span>
        </button>
      </div>
    </div>
  );
    ...
    <div className="pico-step pico-step-form">
      ...
      <div className="pico-info-lines">
        <div className="pico-info-line">
          <IconEmail /> <span>Ihre E-Mail-Adresse für eventuelle Rückfragen.</span>
        </div>
        <div className="pico-info-line">
          <IconNoSpam /> <span>Sie erhalten keine ungefragte Werbung.</span>
        </div>
        <div className="pico-info-line">
          <IconDSGVO /> <span>Ihre Daten werden gemäß DSGVO sicher gespeichert.</span>
        </div>
      </div>
...
function Step5a() {
  return (
    <div className="pico-step pico-step-center">
      <IconCheckLarge />
      <h2 className="pico-step-title" style={{ marginTop: 16 }}>Vielen Dank!</h2>
...
function Step5b() {
  return (
    <div className="pico-step pico-step-center">
      <IconWarningLarge />
      <h2 className="pico-step-title" style={{ marginTop: 16 }}>Adresse nicht gefunden</h2>
      <p className="pico-step-subtitle">
        Leider konnten wir kein Gebäude unter dieser Adresse finden. Bitte überprüfen Sie Ihre Eingabe oder kontaktieren Sie uns direkt.
      </p>
    </div>
  );
}
...
function App() {
  ...
  useEffect(() => {
    ...
    script.src = 'https://homezerotech.github.io/Widget/Production/embed.js';
    script.defer = true;
  ...
  useEffect(() => {
    if (hzContainerRef.current && !hzContainerRef.current.querySelector('hz-embed')) {
  ...
  const goBack = () => {
    ...
    if (prev) {
      ...
      if (step === '4a' || step === '4b') {
        setFormData((p) => ({ ...p, address: '', street: '', housenumber: '', zipcode: '', city: '', email: '', firstName: '', lastName: '', phone: '', privacyChecked: false }));
      }
      setStep(prev);
    }
  };
  ...
  const handleStep4aSubmit = () => {
    try {
      ...
      if (hzEmbed) {
        ...
        const input = root.querySelector('input[type="text"], input[type="search"], input');
        ...
        const emailInputs = root.querySelectorAll('input[type="email"], input[placeholder*="mail"]');
      ...
      console.warn('hz-embed interaction failed, opening URL directly:', err);
    }

    /* Fallback / parallel: construct URL and open in new tab */
    const params = new URLSearchParams();
    if (formData.street) params.set('street', formData.street);
    if (formData.housenumber) params.set('housenumber', formData.housenumber);
    ...
    const baseUrl = 'https://configurator.homezero.nl/link/start?id=3826d61f-6790-4fca-ab8d-e0891ae4d9d9';
    ...
    <div className="pico-widget">
      ...
      <div className="pico-widget-header">
        <div className="pico-header-left">
          {showBack && (
            <button className="pico-btn-icon" onClick={goBack} aria-label="Zurück">
              <IconBack />
            </button>
          )}
        </div>
        ...
        <div className="pico-header-right">
          {showClose && (
            <button className="pico-btn-icon" onClick={handleClose} aria-label="Schließen">
              <IconClose />
            </button>
          )}
        </div>
