...
### 3.1 Data Submission Strategy

When it is desired to send all filled-in information, it can be done in two ways:

1.  **Public Pico API**: Ask to send all information as an **HTML list** in the `AssignmentDescription` field.
2.  **HomeZero Widget**: An extra HTML attribute must be set dynamically, also with an **HTML list as a string**. This is the `data-context` attribute.
