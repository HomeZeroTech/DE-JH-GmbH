...
Once you are happy with the widget, you will need to compile the code (compress all the code into a single optimized file) you can do this by running the following command: `npm run build`. You can also ask the agent to run it for you.

Once that is done, you can deploy it by committing the changes and then pushing them to GitHub. You can do this by clicking on the Source Control icon (the icon with the three dots and lines in between them) and then adding a commit message and clicking **"Commit"**. Then click on the **"Sync Changes"** button. This will push the changes to GitHub and deploys the widget code publically so that it can be used.
